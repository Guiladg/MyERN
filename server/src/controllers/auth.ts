...
class AuthController {
	static login = async (req: Request, res: Response) => {
		...
		// Removes unnecessary information from return data
		const { idUser, password: pass, refreshToken, createdAt, updatedAt, ...retData } = user;
	...
	static resetPassword = async (req: Request, res: Response) => {
		...
		try {
			resetToken = await ResetToken.findOneOrFail({ where: { idUser: user.idUser } });
		} catch (error) {
			resetToken = new ResetToken();
			resetToken.idUser = user.idUser;
	...
	static restorePassword = async (req: Request, res: Response) => {
		...
		let resetToken: ResetToken;
		try {
			resetToken = await ResetToken.findOneOrFail({ where: { idUser: user.idUser, token: token } });
		} catch (error) {
			return res.status(401).send();
		}
